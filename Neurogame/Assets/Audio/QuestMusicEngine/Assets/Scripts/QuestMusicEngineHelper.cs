...
public class QuestMusicEngineHandle {
    ...
    [DllImport("QuestMusicEngine", CallingConvention = CallingConvention.StdCall)]
    ...
    internal QuestMusicEngineHandle(int key) {
        PluginKey = key;
    }
    
    ~QuestMusicEngineHandle() {
    ...
    private static QuestMusicEngineHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (QuestMusicEngineHandle)gch.Target;
    }
...
public class QuestMusicEngineHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static QuestMusicEngineHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("QuestMusicEngineHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                QuestMusicEngineHelper helper = go.AddComponent<QuestMusicEngineHelper>();
                helper.Plugin = new QuestMusicEngineHandle(key);
                return helper;
            }
            return go.GetComponent<QuestMusicEngineHelper>();
        }
    }

    public QuestMusicEngineHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            QuestMusicEngineHandle.RegisterGlobalTransport(null);
        }
    }
}
