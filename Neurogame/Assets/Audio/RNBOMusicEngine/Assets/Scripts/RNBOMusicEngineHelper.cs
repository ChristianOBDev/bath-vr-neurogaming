...
public class RNBOMusicEngineHandle {
    ...
    [DllImport("RNBOMusicEngine", CallingConvention = CallingConvention.StdCall)]
    ...
    internal RNBOMusicEngineHandle(int key) {
        PluginKey = key;
    }
    
    ~RNBOMusicEngineHandle() {
    ...
    private static RNBOMusicEngineHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (RNBOMusicEngineHandle)gch.Target;
    }
...
public class RNBOMusicEngineHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static RNBOMusicEngineHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("RNBOMusicEngineHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                RNBOMusicEngineHelper helper = go.AddComponent<RNBOMusicEngineHelper>();
                helper.Plugin = new RNBOMusicEngineHandle(key);
                return helper;
            }
            return go.GetComponent<RNBOMusicEngineHelper>();
        }
    }

    public RNBOMusicEngineHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            RNBOMusicEngineHandle.RegisterGlobalTransport(null);
        }
    }
}
