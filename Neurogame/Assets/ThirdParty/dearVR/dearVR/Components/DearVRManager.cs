    ...
    public class DearVRManager : MonoBehaviour
    {
        ...
        public static DearVRManager Instance
        {
            get
            {
                if (instance_ == null)
                {
                    instance_ = FindAnyObjectByType<DearVRManager>();
        ...
        void AwakeSingleton()
        {
            if (DearListener == null)
            {
                ...
                {
                    DearListener = FindAnyObjectByType<AudioListener>();
                }
                else
                {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                foreach (string basename in filesNeeded)
                {
                    ...
                    {
                        UnityWebRequest unpackerRequest =
                             UnityWebRequest.Get("jar:file://" + Application.dataPath + "!/assets/dearVR/data/" + basename);

                        UnityWebRequestAsyncOperation operation = unpackerRequest.SendWebRequest();

                        while (!operation.isDone)
                        {
                        } // This will block in the webplayer.

                        if (!string.IsNullOrEmpty(unpackerRequest.error))
                        {
                            pathToData = "";
                            break;
                        }
                        ...
                        File.WriteAllBytes(pathToData + basename,
                            unpackerRequest.downloadHandler.data); // 64MB limit on File.WriteAllBytes.
                    }
                }
