...
public class BasicFPCC : MonoBehaviour
{
  ...
  public InputActionReference actionRun;
  public InputActionReference actionCrouch;
  public InputActionReference actionJump;
  public InputActionReference actionSlide;
  public InputActionReference actionToggleCursor;
  private List<InputActionReference> inputActions; // for easy enable/disable
  ...
  private void Awake()
  {
    ...
    inputActions = new List<InputActionReference> { actionLook, actionMove, actionRun, actionCrouch, actionJump, actionSlide, actionToggleCursor };
  }
  ...
  void OnEnable()
  {
    if (useLocalInputs)
    {
      EnableAction(activateLookAction);
      EnableAction(actionLook);
      EnableAction(actionMove);
      EnableAction(actionRun);
      EnableAction(actionCrouch);
      EnableAction(actionJump);
      EnableAction(actionSlide);
      EnableAction(actionToggleCursor);
    }
  }

  void OnDisable()
  {
    DisableAction(actionLook);
    DisableAction(actionMove);
    DisableAction(actionRun);
    DisableAction(actionCrouch);
    DisableAction(actionJump);
    DisableAction(actionSlide);
    DisableAction(actionToggleCursor);
  }
  ...
  void ProcessInputs()
  {
    if (useLocalInputs)
    {
      if (actionLook != null && actionLook.action != null && activateLookAction != null && activateLookAction.action != null && activateLookAction.action.IsPressed())
      {
        Vector2 look = actionLook.action.ReadValue<Vector2>();
        inputLookX = look.x;
        inputLookY = look.y;
      }


      if (actionMove != null && actionMove.action != null)
      {
        Vector2 move = actionMove.action.ReadValue<Vector2>();
        inputMoveX = move.x;
        inputMoveY = move.y;
      }


      if (actionRun != null && actionRun.action != null)
      {
        inputKeyRun = actionRun.action.IsPressed();
      }


      if (actionCrouch != null && actionCrouch.action != null)
      {
        inputKeyCrouch = actionCrouch.action.IsPressed();
      }


      if (actionJump != null && actionJump.action != null)
      {
        inputKeyDownJump = actionJump.action.WasPressedThisFrame();
      }


      if (actionSlide != null && actionSlide.action != null)
      {
        inputKeyDownSlide = actionSlide.action.WasPressedThisFrame();
      }


      if (actionToggleCursor != null && actionToggleCursor.action != null)
      {
        inputKeyDownCursor = actionToggleCursor.action.WasPressedThisFrame();
      }
    }
  ...
  void ProcessMovement()
  {
    ...
    isSlipping = groundSlopeAngle > controller.slopeLimit;

    // - Check Ceiling above for Head Room -
    // CeilingCheck(); // DISABLED BECAUSE THERE ARE CEILINGS EVERYWHERE
  ...
  void GroundCheck()
  {
    ...
    // FIRST RAYCAST
    if (Physics.Raycast(origin + rayOriginOffset1, Vector3.down, out RaycastHit slopeHit1, raycastLength))
    {
      ...
      // 2ND RAYCAST
      if (Physics.Raycast(origin + rayOriginOffset2, Vector3.down, out RaycastHit slopeHit2, raycastLength))
      {
